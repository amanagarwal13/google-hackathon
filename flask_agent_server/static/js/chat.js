/**
 * ADK Agent Chat Interface JavaScript - Multi-Agent Support
 */
...
let currentAgent = CURRENT_AGENT || 'oracle_agent';
...
function initializeSocket() {
    ...
    socket.on('session_history', (data) => {
        ...
        if (data.messages && data.messages.length > 0) {
            data.messages.forEach(msg => {
                if (msg.role === 'user') {
                    addMessage(msg.content, 'user', msg.timestamp, msg.agent);
                } else {
                    addMessage(msg.content, 'assistant', msg.timestamp, msg.agent);
                }
            });
        }
    });
    ...
    socket.on('agent_response', (data) => {
        addMessage(data.response, 'assistant', data.timestamp, data.agent);
        hideTypingIndicator();
    });
...
async function switchAgent(agentId) {
    ...
    showLoadingOverlay('Switching agent...');
    ...
    try {
        ...
        if (data.success) {
            // Redirect to new session with selected agent
            window.location.href = data.redirect_url;
        } else {
            console.error('Failed to switch agent:', data.error);
            hideLoadingOverlay();
            alert('Failed to switch agent: ' + data.error);
        }
    } catch (error) {
...
async function sendViaAPI(message) {
    ...
    try {
        ...
        if (data.success) {
            addMessage(data.response, 'assistant', null, data.agent);
        } else {
            addMessage(`Error: ${data.error}`, 'error');
        }
    } catch (error) {
...
// Add message to chat
function addMessage(content, type, timestamp = null, agent = null) {
    ...
    const avatarIcon = type === 'user' ? 'fa-user' : (getAgentIcon(agent) || 'fa-robot');
    const author = type === 'user' ? 'You' : (getAgentName(agent) || 'Agent');
    ...
    messageDiv.innerHTML = `
        ...
        <div class="message-content">
            <div class="message-header">
                ...
                ${agent && type === 'assistant' ? `<span class="message-agent">${getAgentName(agent)}</span>` : ''}
            </div>
...
function getAgentIcon(agentId) {
    const iconMap = {
        'oracle_agent': 'fa-crystal-ball',
        'tax_advisor_agent': 'fa-calculator'
    };
...
// New chat
function newChat() {
    // Generate new session ID and reload with current agent
    window.location.href = `/?agent=${currentAgent}`;
}
...
function downloadChat() {
    ...
    chatText += `Agent: ${getAgentName(currentAgent) || 'Unknown'}\n\n`;
    ...
    a.href = url;
    a.download = `chat-export-${currentAgent}-${Date.now()}.txt`;
