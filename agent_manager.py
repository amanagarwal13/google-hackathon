...
class AgentManager:
    ...
    def _load_agent(self):
        ...
        try:
            ...
            # Pattern 1: Import from agent.py
            try:
                agent_module_path = os.path.join(self.agent_path, 'agent.py')
                ...
                logger.warning(f"Failed to load agent from agent.py: {e}")
            
            # Pattern 2: Import from __init__.py
            if not agent_loaded:
                try:
                    agent_package = os.path.basename(self.agent_path)
                    agent_module = __import__(agent_package, fromlist=[''])
                    ...
                    logger.warning(f"Failed to load agent from package: {e}")
            
            if agent_loaded:
                logger.info(f"✅ Successfully loaded agent from {self.agent_path}")
                self._extract_agent_info()
            else:
                raise Exception(f"Could not load agent from {self.agent_path}")
